# App Privacy Nutrition - App Store (Dengo)

## App Privacy Details for App Store Connect

### Data Types Collected

#### 1. Contact Info
**Does the app collect user's contact information?** NO
- Name: NO
- Email: NO
- Phone: NO
- Address: NO
- Other contact details: NO

#### 2. Health & Fitness
**Does the app collect health and fitness information?** NO
- Health data: NO
- Fitness data: NO
- Medical information: NO
- Health records: NO

#### 3. Financial Info
**Does the app collect financial information?** NO
- Payment info: NO
- Credit card data: NO
- Bank account info: NO
- Transaction history: NO

#### 4. Location
**Does the app collect location information?** NO
- Precise location: NO
- Coarse location: NO
- Location history: NO
- Background location: NO

#### 5. Sensitive Info
**Does the app collect sensitive information?** NO
- Race/ethnicity: NO
- Political opinions: NO
- Religious beliefs: NO
- Union membership: NO
- Genetic data: NO
- Biometric data: NO
- Sexual orientation: NO
- Criminal history: NO

#### 6. Contacts
**Does the app collect contacts information?** NO
- Address book: NO
- Contact details: NO
- Social connections: NO

#### 7. Photos or Videos
**Does the app collect photos or videos?** NO
- Photo library: NO
- Camera access: NO
- Video capture: NO
- Photo capture: NO

#### 8. Audio Data
**Does the app collect audio data?** NO
- Microphone access: NO
- Voice recordings: NO
- Audio files: NO

#### 9. Browsing History
**Does the app collect browsing history?** NO
- Web browsing: NO
- App browsing: NO
- Search history: NO

#### 10. Search History
**Does the app collect search history?** NO
- App search: NO
- Content search: NO
- Query history: NO

#### 11. User Content
**Does the app collect user-generated content?** NO
- Photos: NO
- Videos: NO
- Audio: NO
- Text: NO
- Files: NO

#### 12. Identifiers
**Does the app collect identifiers?** YES
- Device ID: YES
- User ID: NO
- Purchase history: NO
- Other identifiers: YES (Analytics identifiers)

#### 13. Purchases
**Does the app collect purchase information?** NO
- In-app purchases: NO
- Subscription data: NO
- Purchase history: NO

#### 14. Usage Data
**Does the app collect usage data?** YES
- App interactions: YES
- Feature usage: YES
- Performance data: YES
- Usage patterns: YES

#### 15. Diagnostics
**Does the app collect diagnostic data?** YES
- Crash reports: YES
- Performance data: YES
- Error reports: YES
- System logs: YES

### Data Use for Tracking

#### Tracking Definition
**Does this app use the ATT framework to track the user?** NO
- Tracking for advertising: NO
- Tracking for data brokering: NO
- Tracking for other purposes: NO

#### Tracking Purposes
- Cross-app advertising tracking: NO
- Ad targeting: NO
- Attribution: NO
- Product personalization: NO
- Analytics: NO
- Other purposes: NO

### Data Linked to User

#### Data Used to Recognize User
**Is data collected used to recognize the user across other companies' services?** NO
- Contact info: NO
- Health & fitness: NO
- Financial info: NO
- Location: NO
- Sensitive info: NO
- Contacts: NO
- Photos/videos: NO
- Audio data: NO
- Browsing history: NO
- Search history: NO
- User content: NO
- Identifiers: NO
- Purchases: NO
- Usage data: NO
- Diagnostics: NO

### Data Used to Contact User

#### Contact for Marketing
**Is data used to contact the user for marketing purposes?** NO
- Contact info: NO
- Health & fitness: NO
- Financial info: NO
- Location: NO
- Sensitive info: NO
- Contacts: NO
- Photos/videos: NO
- Audio data: NO
- Browsing history: NO
- Search history: NO
- User content: NO
- Identifiers: NO
- Purchases: NO
- Usage data: NO
- Diagnostics: NO

### Data Shared with Third Parties

#### Data Sharing Purposes
**Is data shared with third parties?** NO
- App functionality: NO
- Service providers: NO
- Analytics: NO
- Advertising: NO
- Developer's choice: NO

#### Third Party Sharing Details
- Contact info: NO
- Health & fitness: NO
- Financial info: NO
- Location: NO
- Sensitive info: NO
- Contacts: NO
- Photos/videos: NO
- Audio data: NO
- Browsing history: NO
- Search history: NO
- User content: NO
- Identifiers: NO
- Purchases: NO
- Usage data: NO
- Diagnostics: NO

### Privacy Details Summary

#### Data Collection Summary
- **Data collected for App Functionality:** YES (Usage data, Diagnostics)
- **Data collected for Third Party Processors:** NO
- **Data collected for Analytics:** NO (No third-party analytics)
- **Data collected for Product Personalization:** YES (App usage data only)
- **Data collected for Advertising or Marketing:** NO
- **Data collected for Other Purposes:** NO

#### Data Use Summary
- **Data used for Tracking:** NO
- **Data used to Contact User:** NO
- **Data used to Recognize User Across Services:** NO
- **Data Shared with Third Parties:** NO

### Privacy Policy Link
**Privacy Policy URL:** [PLACEHOLDER - Privacy policy URL]

### Compliance Statement
Dengo complies with Apple's App Store privacy requirements and maintains strict privacy standards to protect user information. We collect only the minimum data necessary to provide and improve our service, with clear user consent where required.

### Data Collection Details

#### YES - Usage Data
- **Purpose:** Personalize content recommendations
- **Type:** App usage patterns, feature interactions
- **Retention:** As long as necessary for service provision
- **User Control:** Available through app settings

#### YES - Diagnostics
- **Purpose:** Improve app performance and stability
- **Type:** Crash reports, error logs, performance metrics
- **Retention:** Limited time for debugging purposes
- **User Control:** Available through device settings

#### YES - Identifiers
- **Purpose:** Analytics and app functionality
- **Type:** Device identifiers, analytics identifiers
- **Retention:** As long as necessary for service provision
- **User Control:** Available through device settings